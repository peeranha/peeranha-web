...
const Menu = ({ sort, sorting }) => (
  <Ul>
    {Object.values({ all: { value: null, label: 'all' }, ...QUESTION_TYPES }).map(x => (
      <CheckedItem key={x.label} onClick={() => sort(x.value)} isActive={x.value === sorting}>
...
export default React.memo(QuestionTypeDropdown);
