...
import {
  ...
  showKeycatSignUpFormErr,
  showKeycatSignUpFormSuccess,
  signUpWithKeycatSuccess,
  signUpWithKeycatErr,
} from './actions';
...
export function* signUpWithKeycatWorker({ val }) {
  try {
    const accountName = val[KEYCAT_ACCOUNT_FIELD];
    const profile = {
      accountName,
      displayName: val[DISPLAY_NAME_FIELD],
    };
    ...
    yield call(loginWithKeycatWorker);
    ...
    yield put(signUpWithKeycatSuccess());

    yield call(createdHistory.push, routes.questions());
  } catch (err) {
    yield put(signUpWithKeycatErr(err));
  }
}
...
export function* showKeycatSignUpFormWorker({ loginAccountName = null }) {
  try {
    let currentAccountName = loginAccountName;

    const locale = yield select(makeSelectLocale());
    const translations = translationMessages[locale];

    // if not redirected from login form
    if (!loginAccountName) {
      const eosService = yield select(selectEos);
      const keycatAccountData = yield call(eosService.keycatSignIn);

      const { accountName } = keycatAccountData;
      const profileInfo = yield call(getProfileInfo, accountName, eosService);
    ...
    yield put(getUserProfileSuccess(currentAccountName));

    yield put(showKeycatSignUpFormSuccess(currentAccountName));

    yield call(createdHistory.push, routes.signup.keycatDisplayName.name);
  } catch (err) {
    yield put(showKeycatSignUpFormErr(err));
  }
}
...
export default function*() {
  ...
  yield takeLatest(SIGNUP_WITH_KEYCAT, signUpWithKeycatWorker);
  yield takeLatest(SHOW_SCATTER_SIGNUP_FORM, showScatterSignUpFormWorker);
  yield takeLatest(SHOW_KEYCAT_SIGNUP_FORM, showKeycatSignUpFormWorker);
  yield takeLatest(
