...
function viewQuestionReducer(state = initialState, action) {
  ...
  switch (type) {
    ...
    case GET_QUESTION_DATA_SUCCESS:
      return state.set('questionDataLoading', false).set('questionData', questionData);
    ...
    case POST_ANSWER_ERROR:
      return state.set('postAnswerLoading', false).set('postAnswerError', postAnswerError);

    case SHOW_ADD_COMMENT_FORM:
      return state.updateIn(['addCommentFormDisplay'], (arr) => arr.push(toggleFormButtonId));

    case HIDE_ADD_COMMENT_FORM:
      return state.set(
        'addCommentFormDisplay',
        state.get('addCommentFormDisplay').filterNot((el) => el === toggleFormButtonId),
      );

    case POST_COMMENT:
      return state.set('postCommentLoading', true).set('ids', [...state.toJS().ids, buttonId]);
    case POST_COMMENT_SUCCESS:
      return state.set('postCommentLoading', false).set(
        'ids',
    ...
    case DELETE_QUESTION:
      return state.set('deleteQuestionLoading', true).set('ids', [...state.toJS().ids, buttonId]);
    case DELETE_QUESTION_SUCCESS:
      return state.set('deleteQuestionLoading', false).set(
        'ids',
        state.toJS().ids.filter((x) => x !== buttonId),
      );
    ...
    case PAY_BOUNTY_SUCCESS:
      return state.set('giveBountyLoading', false).set(
        'ids',
        state.toJS().ids.filter((x) => x !== buttonId),
      );
    ...
    case GET_QUESTION_BOUNTY_SUCCESS:
      return state.set('questionBountyLoading', false).set('questionBounty', questionBounty);
