...
export const Answers = ({ questionData: { answers, ...questionData }, ...props }) => {
  if (!answers.length || !answers[0].content) return null;

  const updatedQuestionData = useMemo(() => {
    const sortedByRatingAnswers = _orderBy(answers, 'rating', 'desc').map((x) => ({
      ...x,
    ...
    const officialAnswers = sortedByRatingAnswers.filter((answer) => !!answer.isOfficialReply);

    const correctAnswer = sortedByRatingAnswers.find(({ isBestReply }) => isBestReply);
    const officialAnswerIds = officialAnswers.map(({ id }) => id);
    const rest = sortedByRatingAnswers.filter(
      ({ id }) => !(officialAnswerIds.includes(id) || id === correctAnswer?.id),
    );
    return {
      ...questionData,
      answers: _uniqBy(officialAnswers.concat([correctAnswer]).concat(rest).filter(Boolean), 'id'),
    };
