  ...
  createSelector(selectQuestionsWithAnswersOfUserDomain, (substate) => {
    ...
    if (sortedQuestByAnswDate.length > 1) {
      sortedQuestByAnswDate = sortedQuestByAnswDate.sort((quest1, quest2) => {
        const asnwDate1 = quest1.replies.find((el) => el.id === quest1.answerId)?.postTime;
        const asnwDate2 = quest2.replies.find((el) => el.id === quest2.answerId)?.postTime;
...
const selectNumber = () =>
  createSelector(selectQuestionsWithAnswersOfUserDomain, (substate) => substate.toJS().number);
...
const selectIsLastFetch = () =>
  createSelector(selectQuestionsWithAnswersOfUserDomain, (substate) => substate.toJS().isLastFetch);
