...
export const getBalance = async (ethereumService, user) => {
  if (user) {
    const balance = await ethereumService.getTokenDataWithArgs(GET_USER_BALANCE, [user]);
...
export const getAvailableBalance = async (ethereumService, user) => {
  if (user) {
    const balance = await ethereumService.getTokenDataWithArgs(GET_AVAILABLE_BALANCE, [user]);
    return Number(formatEther(balance));
  }
  return 0;
};
...
export async function getWeekStat(ethereumService, userId) {
  ...
  if (user?.creationTime) {
    return weekStat.filter((period) => Number(user.creationTime) < Number(period.periodFinished));
  }
  return weekStat;
}

export async function pickupReward(ethereumService, user, periodIndex) {
  return await ethereumService.sendTransaction(CONTRACT_TOKEN, user, CLAIM_REWARD, [periodIndex]);
}
...
export async function getUserBoostStatistics(ethereumService, user, currentPeriod) {
  const averageStakeCurrent =
    (await ethereumService.getTokenDataWithArgs(GET_AVERAGE_STAKE, [currentPeriod])) / WEI_IN_ETH;
  const averageStakeNext =
    (await ethereumService.getTokenDataWithArgs(GET_AVERAGE_STAKE, [currentPeriod + 1])) /
    WEI_IN_ETH;

  const availableBalance =
    (await ethereumService.getTokenDataWithArgs(GET_AVAILABLE_BALANCE, [user])) / WEI_IN_ETH;

  const userStakeCurrent =
    (await ethereumService.getTokenDataWithArgs(GET_USER_STAKE, [user, currentPeriod])) /
    WEI_IN_ETH;
  const userStakeNext =
    (await ethereumService.getTokenDataWithArgs(GET_USER_STAKE, [user, currentPeriod + 1])) /
    WEI_IN_ETH;

  const userBoostCurrent =
    (await ethereumService.getTokenDataWithArgs(GET_BOOST, [user, currentPeriod])) / 1000;
  const userBoostNext =
    (await ethereumService.getTokenDataWithArgs(GET_BOOST, [user, currentPeriod + 1])) / 1000;

  const totalPeriodStake = await ethereumService.getTokenDataWithArgs(GET_STAKE, [
    currentPeriod + 1,
  ]);
...
export async function getUserBoost(ethereumService, user, period) {
  return Number((await ethereumService.getTokenDataWithArgs(GET_BOOST, [user, period])) / 1000);
}

export async function getUserStake(ethereumService, user, period) {
  return Number(
    (await ethereumService.getTokenDataWithArgs(GET_USER_STAKE, [user, period])) / WEI_IN_ETH,
  );
}

export async function addBoost(ethereumService, user, tokens) {
  return await ethereumService.sendTransaction(CONTRACT_TOKEN, user, SET_STAKE, [user, tokens]);
}
...
export function calculateNewBoost(userBoostStat, newStake) {
  ...
  const totalStake = Number(userBoostStat.totalPeriodStake[0].toString() / WEI_IN_ETH);
  const totalUsers = Number(userBoostStat.totalPeriodStake[1].toString());

  if (userBoostStat.userStakeNext > 0) {
    newAverageStake = (totalStake - userBoostStat.userStakeNext + newStake) / totalUsers;
  } else {
    newAverageStake = (totalStake + newStake) / (totalUsers + 1);
  }

  if (newStake <= newAverageStake) {
    predictedBoost = (newStake / newAverageStake) * BOOST_MULTIPLIER + BOOST_MODIFIER_LOW;
  } else {
